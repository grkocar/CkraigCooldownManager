    ...
    if child.GetRegions then
        for i = 1, select('#', child:GetRegions()) do
            ...
            if region and region.IsObjectType and region:IsObjectType("Texture") then
                ...
                -- Optionally check for border-like texcoords or names
                region:SetAlpha(0)
            end
        end
    end
...
local function GetSpellCandidatesFromCooldownInfo(cooldownInfo)
    local candidates = {}
    if not cooldownInfo then return candidates end
...
local function ResolveBarSpellIdentity(barFrame, fallbackIndex)
    ...
    local sources = {
        barFrame,
        barFrame.Bar,
        barFrame.cooldownInfo,
        barFrame.Bar and barFrame.Bar.cooldownInfo,
    }
...
local function ApplyBarClusterDragState(settings)
    ...
    for i = 1, MAX_BAR_CLUSTERS do
        ...
        if anchor then
            ...
            if (unlocked or ccm_inEditMode) and inUse then
...
local function SetupEditModeCallbacks()
    ...
    -- Enter Edit Mode: show in-use cluster anchors so they're selectable/draggable
    LibEditMode:RegisterCallback("enter", function()
        ccm_inEditMode = true
        local settings = CkraigCooldownManager.db and CkraigCooldownManager.db.profile and CkraigCooldownManager.db.profile.buffBars
        if not settings or not settings.clusterMode then return end
        local clusterCount = settings.clusterCount or 3
...
function CkraigCooldownManager:StyleBar(barFrame)
    if not barFrame then return end
    -- Skip if bar was already styled at current version
    local bs = GetBarState(barFrame)
    if bs.styleVer == _bb_styleVersion then return end
    ...
    if iconContainer then
        local iconW, iconH
        if barFrame._bb_clusterBarHeight then
            iconW = barFrame._bb_clusterBarHeight
            iconH = barFrame._bb_clusterBarHeight
        elseif isVerticalBar then
            -- For vertical bars, make icon square matching the bar's narrow width
            local parentW = barFrame._bb_verticalBarWidth or parent:GetWidth() or barHeight
            iconW = parentW
            iconH = parentW
        else
...
function CkraigCooldownManager:RepositionAllBars()
    ...
    local settings = self.db.profile.buffBars
    -- Always bump style version to force restyle (fixes first load skin issue)
    _bb_styleVersion = _bb_styleVersion + 1
    local bars = GetBuffBarFrames()
    ...
    if settings.clusterMode then
        ...
        for i, bar in ipairs(bars) do
            ...
            if spellKey then
                -- Keep insertion order stable: first shown bar stays at the base,
                -- newly seen bars are appended and therefore grow upward.
            ...
            if anchor then
                ...
                if flow == "vertical" then
                    ...
                    for bi, entry in ipairs(orderedBars) do
                        ...
                        if bar.SetPoint then pcall(bar.SetPoint, bar, "BOTTOMLEFT", anchor, "BOTTOMLEFT", x + 4, 16) end
                        ...
                        barObj._bb_verticalBarWidth = vertBarWidth
                        ...
                        if bar.SetSize then pcall(bar.SetSize, bar, vertBarWidth, vertBarHeight) end
                    ...
                    local totalHeight = 0
                    local growDir = string.lower(tostring(settings.clusterGrowDirections[ci] or "up"))
                    ...
                    for bi, entry in ipairs(orderedBars) do
                        ...
                        if bar.SetPoint then
                            if growDown then
                                pcall(bar.SetPoint, bar, "TOPLEFT", anchor, "TOPLEFT", 4, -(y + 16))
                            else
                                pcall(bar.SetPoint, bar, "BOTTOMLEFT", anchor, "BOTTOMLEFT", 4, y + 16)
                            end
                        end
                        if bar.SetFrameStrata then pcall(bar.SetFrameStrata, bar, "MEDIUM") end
                        ...
                        barObj._bb_verticalBarWidth = nil
                        barObj._bb_iconPosition = nil
                    ...
                    -- Keep horizontal anchor height fixed so the first bar's
                    -- screen position does not shift as bars appear/disappear.
                    if anchor.SetWidth then
                        anchor:SetWidth(clusterBarWidth)
                    end
                end
                anchor:Show()
            end
    ...
    elseif settings.gridMode then
        ...
        for i, bar in ipairs(bars) do
            ...
            if i <= maxBars then
                ...
                if bar.SetPoint then pcall(bar.SetPoint, bar, "BOTTOMLEFT", BuffBarCooldownViewer, "BOTTOMLEFT", 0, y) end
                local barObj = bar.Bar or bar
                barObj._bb_barIndex = i
                local spellKey, spellLabel = ResolveBarSpellIdentity(bar, i)
                barObj._bb_spellKey = spellKey
    ...
    for frame in pool:EnumerateActive() do
        ...
        if spellKey then
            ...
            if glowCfg and glowCfg.enabled and not frame.auraDataUnit then
                ...
                if shouldGlow then
                    if not glowTarget._bb_glowing then
...
-- Merged single-pass: fade + glow in one pool iteration (replaces separate CheckFadingBars + UpdateBarGlows on dispatch)
...
function CkraigCooldownManager:OnEnable()
    ...
    if not self._bb_updateFrame then
        ...
        self._bb_updateFrame:SetScript("OnEvent", function(ef, event, arg1)
            ...
            -- CDM data loaded/spell overrides/world enter: force full restyle
            if event == "COOLDOWN_VIEWER_DATA_LOADED" or event == "COOLDOWN_VIEWER_SPELL_OVERRIDE_UPDATED" or event == "PLAYER_ENTERING_WORLD" then
                InvalidateBarStyle()
        ...
        if BuffBarCooldownViewer and BuffBarCooldownViewer.RefreshLayout then
            hooksecurefunc(BuffBarCooldownViewer, "RefreshLayout", function()
                self._bb_updateFrame._layoutDirty = true
                self._bb_updateFrame:Show()
            end)
        end

        -- Hook the item container's Layout() — defer to dispatch frame
        ...
        self._bb_updateFrame:SetScript("OnUpdate", function(frame)
            ...
            -- Use actual VISIBLE bar count for change detection, not pool internal count.
            -- Pool can report active items before bars are IsShown/IsVisible, which causes
            -- lastBarCount to latch early and skip RepositionAllBars when bars finally appear.
            local visibleBars = GetBuffBarFrames()
            local count = #visibleBars
            if count == 0 then return end

            -- Run full layout when visible bar count changed, hooks triggered, or cluster mode
            if count ~= lastBarCount or frame._layoutDirty or self.db.profile.buffBars.clusterMode then
                lastBarCount = count
...
gridRowsBox:SetScript("OnEscapePressed", function(self)
    ...
    clusterDimHeader:SetTextColor(0.9, 0.9, 0.9, 1)
    ccY = ccY - 16
    ...
    local function RebuildClusterDimSliders()
        ...
        for ci = 1, count do
            ...
            hSlider:HookScript("OnValueChanged", function(self, value)
                ...
                if type(InvalidateBarStyle) == "function" then InvalidateBarStyle() end
                if type(CkraigCooldownManager.RepositionAllBars) == "function" then CkraigCooldownManager:RepositionAllBars() end
            end)

            -- Flow toggle button (Horizontal / Vertical)
            local currentFlow = string.lower(tostring(clusterSettings.clusterFlows[ci] or clusterSettings.clusterFlow or "horizontal"))
            local flowBtn = CreateFrame("Button", nil, clusterControls, "UIPanelButtonTemplate")
            flowBtn:SetSize(110, 20)
            flowBtn:SetPoint("TOPLEFT", lbl, "BOTTOMLEFT", 0, -2)
            ...
            flowBtn:SetScript("OnClick", function(self)
                ...
                local cur = string.lower(tostring(clusterSettings.clusterFlows[idx] or clusterSettings.clusterFlow or "horizontal"))
            ...
            -- Icon position toggle button (Bottom / Top / Left)
            local iconPositions = { "bottom", "top", "left" }
            local iconPosLabels = { bottom = "Icon: Bottom", top = "Icon: Top", left = "Icon: Left" }
            local curIconPos = clusterSettings.clusterIconPositions[ci] or "bottom"
            ...
            iconPosBtn:SetText(iconPosLabels[curIconPos] or "Icon: Bottom")
            iconPosBtn._clusterIdx = ci
            iconPosBtn:SetScript("OnClick", function(self)
                local idx = self._clusterIdx
                local cur = clusterSettings.clusterIconPositions[idx] or "bottom"
                local nextIdx = 1
                for k, v in ipairs(iconPositions) do
                    if v == cur then nextIdx = (k % #iconPositions) + 1; break end
                end
                local newPos = iconPositions[nextIdx]
                clusterSettings.clusterIconPositions[idx] = newPos
                self:SetText(iconPosLabels[newPos] or "Icon: Bottom")
                if type(CkraigCooldownManager.RepositionAllBars) == "function" then CkraigCooldownManager:RepositionAllBars() end
            end)

            local growLabels = {
                up = "Grow: Bottom->Top",
                down = "Grow: Top->Bottom",
            }
            local curGrow = string.lower(tostring(clusterSettings.clusterGrowDirections[ci] or "up"))
            if curGrow ~= "down" then curGrow = "up" end
            local growBtn = CreateFrame("Button", nil, clusterControls, "UIPanelButtonTemplate")
            growBtn:SetSize(130, 20)
            growBtn:SetPoint("LEFT", iconPosBtn, "RIGHT", 6, 0)
            growBtn:SetText(growLabels[curGrow])
            growBtn._clusterIdx = ci
            growBtn:SetScript("OnClick", function(self)
                local idx = self._clusterIdx
                local cur = string.lower(tostring(clusterSettings.clusterGrowDirections[idx] or "up"))
                local nextDir = (cur == "down") and "up" or "down"
                clusterSettings.clusterGrowDirections[idx] = nextDir
                self:SetText(growLabels[nextDir])
                if type(CkraigCooldownManager.RepositionAllBars) == "function" then CkraigCooldownManager:RepositionAllBars() end
            end)
            if currentFlow == "vertical" then
                growBtn:Disable()
                growBtn:SetAlpha(0.5)
            end
    ...
    assignHeader:SetTextColor(0.9, 0.9, 0.9, 1)
    ...
    local function RepositionClusterAssignSection()
        local count = clusterSettings.clusterCount or 3
        local sectionY = ccY - (count * 72 + 10)
